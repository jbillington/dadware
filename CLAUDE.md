...
- **A blocked measurement is reported, never a zero** — `~/.Trash` is TCC-protected, and `du` prints a cheerful `0` for a folder it cannot read. `scanners/trash.py` runs the `utils/permissions.py` access probe *first* and lets its answer stand: a blocked location gets `status: 'no_permission'`, stays out of the totals rather than adding a zero to them, and renders as "not measured" with the fix beside it. The section headline follows the same rule — a Trash nobody could read never headlines as "0 B". `item_count` and `oldest_age_days` are `None`, not `0`, for the same reason.
- **The Trash is measured but not graded** — the plan proposed a letter-grade ding over 5 GB. Those bytes already sit inside the Free Space grade, which carries half the composite, so a Trash component would count the same gigabytes twice, and any new component re-baselines every existing tester's grade. It gets an aside on the report card and the first of Dad's tips instead, because it is the one line a reader can act on in a minute.
