...
- **A volume that isn't home's gets a report about that volume** — `run_storage_scan()` asks `is_on_scan_volume()` whether the chosen volume is the one the home folder lives on. When it isn't (a thumb drive, an external disk), the home breakdown, the permission choreography, Full Disk Access, Mac app libraries, hidden caches and snapshots are all skipped, and their keys are left out of `scan_data` — every section renderer already omits a section whose data is absent. `scan_data['scan_scope']` is `'home_volume'` or `'other_volume'`; on `'other_volume'` the report card is titled "Volume Report Card", grades Free Space alone at full weight, and says which sections live on the startup disk. The rule is by *disk*, not by path, so `--volume ~/Downloads` still gets the whole report.
