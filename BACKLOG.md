...
- [ ] **Decide decimal vs binary size units.** Found by the purgeable spike, Aug 24 2026. `format_size()` is 1024-based but labels output "GB", so we print 47.5 GB where Finder prints 50.98 GB for the same bytes — macOS has been decimal since Snow Leopard. A user checking our report against Finder sees ~7% less and concludes the tool is broken. Three options: match Finder (decimal), keep binary, or keep binary with honest GiB labels. Changes every number in every report, so it needs a product call. Research prompt written: `docs/research/COMPETITOR-UX-RESEARCH-PROMPT.md`.
