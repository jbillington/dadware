...
- [ ] **`check_full_disk_access()` only probes three libraries.** Messages, Mail and Photos - so the "couldn't measure" list is that subset, never the full reach of the setting (Trash, other apps' containers, Safari data). The copy now says so explicitly rather than implying the list is exhaustive, but the check itself could cover more. *(The Photos half of this was fixed Sep 2, 2026: the probe listed the `.photoslibrary` bundle - which is not TCC-protected - and swallowed the denial from its internals, so Photos read as accessible with Full Disk Access switched off. It now reads an internal directory and reports what it finds. What else to probe is still open.)*
