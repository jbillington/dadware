...
- [ ] **arm64 is untested — every binary so far is x86_64-only.** Hardware is now available: an M1 to build on and an M4 to scan on (neither runs Tahoe). PyInstaller cannot cross-compile, so a universal2 binary needs a universal2 *interpreter* — Homebrew Python on the M1 would produce an arm64-only binary that will not run on the Intel Mac. Use python.org's universal2 build and assert both slices with `lipo -info`. Watch for Rosetta masking the result on the M4: a thin x86_64 binary runs there under translation and looks fine, so check `sysctl -n sysctl.proc_translated`. CI's universal2 job already does all of this correctly on an Intel runner but is gated to tags and `workflow_dispatch`, so it has never run. Spec: `docs/roadmap/ARCH-COVERAGE-PLAN.md`.
