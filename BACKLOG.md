...
- [ ] **Instrument every phase with a timer, then report an honest total.** Found on the Aug 28 real-Mac run and confirmed with a stopwatch: the volume walk reported 63s while the whole run took **3m 10s**. `duration_seconds` is set inside `scan_storage()`, so it times the volume walk alone — the second home-directory walk, the Mac library scan, the hidden-cache scan, the snapshot check, grading and HTML rendering are all invisible. Two parts:
  1. **Timers on each sub-section** (volume walk, home walk, libraries, caches, snapshots, grading, render), for debugging performance. Behind `DIAGNOSTIC_LOGGING` so a normal run stays quiet, or as a `--timings` flag.
  2. **The reported number becomes wall-clock for the whole run**, measured in `main()`. Today's figure reads as the answer to "how long did that take?" and is wrong by a factor of three.

- [ ] **The home directory is walked twice, and it is most of the disk.** Found Aug 28, 2026 while explaining the run time. `run_storage_scan()` walks the selected volume, then — when the volume isn't home — walks `~` again from scratch for the detailed breakdown, and `merge_home_folders()` swaps the result in. The real numbers from that run: the volume walk found **276,353 items**, the home walk **244,324** — so roughly **88% of the second walk is re-reading files the first walk already stat'd**, and it cost about two of the three minutes.

  **Why it exists:** `depth` controls how deep the *reported* folder breakdown goes, not how deep the walk goes. Scanning `/` at depth 2 buckets everything under `/Users/johnbillington` into one row, so Downloads/Desktop/Documents (depth 3 from `/`) never get their own line — which is the most useful part of the report. The second scan re-roots at `~` so those become depth-1 folders.

  **Options, cheapest first:** (a) keep one walk and let the bucketing go one level deeper *under the home path only*, since `scan_storage()` already visits every file and accumulates per-folder totals — the data is there, it is the bucket key that is wrong; (b) scan home first, then walk the volume with home pruned, and add the two together; (c) scan `/` at depth 3 and filter. Option (a) looks right and needs no second walk at all. Whatever is chosen, the single-pass `os.scandir` design and one-`stat()`-per-file rule stay intact — this is about not walking the same tree twice, not about how each file is measured.
